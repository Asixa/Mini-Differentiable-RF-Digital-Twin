...
def _point_in_triangle_3d(p, v0, v1, v2):
    ...
    Args:
        p: Test point (mi.Point3f)
        v0, v1, v2: Triangle vertices (mi.Point3f)

    Returns:
        mi.Bool: True if point is inside triangle
    """
    # Edge vectors
    edge1 = v1 - v0
    edge2 = v2 - v0
...
def _dda_loop_body(state, params):
    ...
    cell_ix = mi.UInt32(cell_idx % nx)
    cell_iy = mi.UInt32(cell_idx // nx)
    cell_x = dr.gather(mi.Float, x_coords_dr, cell_ix)
    cell_y = dr.gather(mi.Float, y_coords_dr, cell_iy)
    ...
    cell_pos = mi.Point3f(cell_x, cell_y, mi.Float(rx_z))
    ...
    if validate_paths and has_mesh_data:
        ...
        v0 = dr.gather(mi.Point3f, tri_v0, prev_prim_idx)
        v1 = dr.gather(mi.Point3f, tri_v1, prev_prim_idx)
        v2 = dr.gather(mi.Point3f, tri_v2, prev_prim_idx)
        in_main = _point_in_triangle_3d(int_p, v0, v1, v2)

        # Heuristic: use adjacent triangle when faces are split into pairs.
        sibling_idx = prev_prim_idx ^ mi.UInt32(1)
        sibling_idx = dr.select(sibling_idx < n_triangles, sibling_idx, prev_prim_idx)
        sv0 = dr.gather(mi.Point3f, tri_v0, sibling_idx)
        sv1 = dr.gather(mi.Point3f, tri_v1, sibling_idx)
        sv2 = dr.gather(mi.Point3f, tri_v2, sibling_idx)
    ...
    fspl = mi.Float(wavelength) / (4.0 * math.pi * dr.maximum(d_mirror, mi.Float(0.01)))
    field_amplitude = prev_ampl * fspl
    phase = -mi.Float(k) * d_mirror
    ...
    dr.scatter_reduce(dr.ReduceOp.Add, result_real[bounce_idx],
                      dr.select(valid_path, real_contrib, mi.Float(0.0)),
                      cell_idx, valid_path)
    dr.scatter_reduce(dr.ReduceOp.Add, result_imag[bounce_idx],
                      dr.select(valid_path, imag_contrib, mi.Float(0.0)),
                      cell_idx, valid_path)
    dr.scatter_reduce(dr.ReduceOp.Add, result_count[bounce_idx],
                      dr.select(valid_path, mi.Float(1.0), mi.Float(0.0)),
                      cell_idx, valid_path)
...
def _compute_reflection_field_impl(grid, rx_z, tx_pos, mi_scene, wavelength, k,
    ...
    Returns:
        a_ref_total: Total reflection field (Mitsuba Complex2f)
    ...
    if grid_data is not None:
        ...
        idx_x = dr.arange(mi.Float, nx)
        idx_y = dr.arange(mi.Float, ny)
        x_coords_dr = mi.Float(x_min) + idx_x * mi.Float(x_step)
        y_coords_dr = mi.Float(y_min) + idx_y * mi.Float(y_step)
    ...
    results_real = [dr.zeros(mi.Float, n_rx) for _ in range(max_reflections)]
    results_imag = [dr.zeros(mi.Float, n_rx) for _ in range(max_reflections)]
    results_count = [dr.zeros(mi.Float, n_rx) for _ in range(max_reflections)]
    ...
    # tx_pos is mi.Point3f - broadcast to n_rays using dr.repeat (preserves gradients)
    ...
    active = dr.full(mi.Bool, True, n_rays)
    amplitude = dr.ones(mi.Float, n_rays)

    # Previous reflection data for image method (stored per ray)
    prev_refl_p = mi.Point3f(
        dr.zeros(mi.Float, n_rays),
        dr.zeros(mi.Float, n_rays),
        dr.zeros(mi.Float, n_rays)
    )
    prev_refl_n = mi.Vector3f(
        dr.zeros(mi.Float, n_rays),
        dr.zeros(mi.Float, n_rays),
        dr.zeros(mi.Float, n_rays)
    )
    ...
    prev_ampl = dr.ones(mi.Float, n_rays)
    prev_prim_idx = dr.zeros(mi.UInt32, n_rays)
    ...
    for bounce in range(max_reflections + 1):
        ...
        with dr.suspend_grad():
            si = mi_scene.ray_intersect(ray, active)
            hit = si.is_valid() & active
            blocker_dist = dr.select(hit, si.t, mi.Float(1e10))
        # Derive normals/points from triangle data to keep AD path to vertices (e.g., rotation)
        if has_mesh_data:
            prim_idx = dr.select(hit, mi.UInt32(si.prim_index), mi.UInt32(0))
            v0 = dr.gather(mi.Point3f, tri_v0, prim_idx)
            v1 = dr.gather(mi.Point3f, tri_v1, prim_idx)
            v2 = dr.gather(mi.Point3f, tri_v2, prim_idx)
        ...
        if bounce > 0:
            ...
            t = dr.zeros(mi.Float, n_rays)
            dt_x = dr.abs(mi.Float(cell_size_x) / dr.maximum(dr.abs(ray_dir.x), mi.Float(EPS)))
            dt_y = dr.abs(mi.Float(cell_size_y) / dr.maximum(dr.abs(ray_dir.y), mi.Float(EPS)))

            cur_x = ray_origin.x
            cur_y = ray_origin.y

            step_x = dr.select(ray_dir.x > 0, mi.Float(cell_size_x), mi.Float(-cell_size_x))
            step_y = dr.select(ray_dir.y > 0, mi.Float(cell_size_y), mi.Float(-cell_size_y))
            ...
            t_max_x = dr.abs((next_x - cur_x) / dr.maximum(dr.abs(ray_dir.x), mi.Float(EPS)))
            t_max_y = dr.abs((next_y - cur_y) / dr.maximum(dr.abs(ray_dir.y), mi.Float(EPS)))
            ...
            params = (mi.Float(x_min), mi.Float(x_max), mi.Float(y_min), mi.Float(y_max),
                      mi.Float(cell_size_x), mi.Float(cell_size_y), nx,
                      x_coords_dr, y_coords_dr, rx_z, wavelength, k,
                      validate_paths, has_mesh_data, n_triangles, tri_v0, tri_v1, tri_v2,
        ...
        prev_prim_idx = dr.select(hit, mi.UInt32(si.prim_index), prev_prim_idx)
    ...
    for i in range(max_reflections):
        ...
        avg_real = dr.select(mask, results_real[i] / results_count[i], mi.Float(0.0))
        avg_imag = dr.select(mask, results_imag[i] / results_count[i], mi.Float(0.0))
        a_ref = mi.Complex2f(avg_real, avg_imag)
        a_ref_list.append(a_ref)

    # Sum for total
    a_ref_total = mi.Complex2f(0, 0)
...
def compute_reflection_field(grid, rx_z, tx_pos, scene, wavelength, k,
    ...
    if scene is None:
        ...
        n_rx = grid.n_cells
        zero_real = dr.zeros(mi.Float, n_rx)
        zero_imag = dr.zeros(mi.Float, n_rx)
        zero_field = mi.Complex2f(zero_real, zero_imag)
        if return_per_bounce:
